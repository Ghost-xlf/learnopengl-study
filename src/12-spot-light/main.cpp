...
int main()
{
  ...
  Shader ourShade("./src/12-spot-light/shader/vertex.glsl", "./src/12-spot-light/shader/fragment.glsl");
  Shader lightObjectShader("./src/12-spot-light/shader/light_object_vert.glsl", "./src/12-spot-light/shader/light_object_frag.glsl");
  ...
  unsigned texture0, texture1;
  stbi_set_flip_vertically_on_load(true); // 纹理采样y轴取反
  glGenTextures(1, &texture0);            // 此处的索引标识符不是纹理单元是生成的纹理名称
  glActiveTexture(GL_TEXTURE0);           // 如果此地不绑定对应纹理单元  默认绑定至 GL_TEXTURE0
  ...
  if (data)
  {
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data); // 注意图片是RGBA格式
  ...
  glm::vec3 lightColor = glm::vec3(1.0, 1.0, 1.0);    // 光照颜色
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    ImGui::SliderFloat("ambient", (float *)&ambientStrength, 0.0f, 2.0f);
    ...
    ourShade.setVec3("light.position",  camera.Position);
    ourShade.setVec3("light.direction", camera.Front);
    ourShade.setFloat("light.constant",  1.0f);
    ourShade.setFloat("light.linear",    0.09f);
    ourShade.setFloat("light.quadratic", 0.032f);
    ourShade.setFloat("light.cutOff",   glm::cos(glm::radians(12.5f)));
    ourShade.setFloat("light.outerCutOff",   glm::cos(glm::radians(17.5f)));
